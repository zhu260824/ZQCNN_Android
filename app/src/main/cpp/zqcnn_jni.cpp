...
static ZQMTCNN *zqmtcnn;

extern "C" {

JNIEXPORT jboolean JNICALL
Java_com_zl_demo_ZQMTCNN_initModelPath(JNIEnv *env, jobject instance, jstring modelPath_) {
    if (NULL == modelPath_) {
        return false;
    }
    ...
    zqmtcnn = new ZQMTCNN(tFaceModelDir);
    return true;
}

JNIEXPORT jfloatArray JNICALL
Java_com_zl_demo_ZQMTCNN_detectFace(JNIEnv *env, jobject instance,jstring imgPath_) {
    const char *imgPath = env->GetStringUTFChars(imgPath_, 0);
    vector<ZQ_CNN_BBox> faceInfo = zqmtcnn->detect(imgPath);
